...
import (
	"database/sql"
	"errors"
	"github.com/azar-intelops/microservice-demo/one2oneclient/pkg/rest/server/daos/clients/sqls"
...
func (deviceDao *DeviceDao) CreateDevice(m *models.Device) (*models.Device, error) {
	insertQuery := "INSERT INTO devices(Year)values(?)"
	res, err := deviceDao.sqlClient.DB.Exec(insertQuery, m.Year)
	if err != nil {
		return nil, err
	}
...
func (deviceDao *DeviceDao) UpdateDevice(id int64, m *models.Device) (*models.Device, error) {
	...
	updateQuery := "UPDATE devices SET Year = ? WHERE Id = ?"
	res, err := deviceDao.sqlClient.DB.Exec(updateQuery, m.Year, id)
	if err != nil {
		return nil, err
	}
...
func (deviceDao *DeviceDao) ListDevices() ([]*models.Device, error) {
	...
	for rows.Next() {
		m := models.Device{}
		if err = rows.Scan(&m.Id, &m.Year); err != nil {
			return nil, err
		}
...
func (deviceDao *DeviceDao) GetDevice(id int64) (*models.Device, error) {
	...
	m := models.Device{}
	if err := row.Scan(&m.Id, &m.Year); err != nil {
