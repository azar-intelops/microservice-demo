...
import (
	"database/sql"
	"errors"
	"github.com/azar-intelops/microservice-demo/one2one/pkg/rest/server/daos/clients/sqls"
	"github.com/azar-intelops/microservice-demo/one2one/pkg/rest/server/models"
...
func (userDao *UserDao) CreateUser(m *models.User) (*models.User, error) {
	...
	res, err := userDao.sqlClient.DB.Exec(insertQuery, m.Username)
	if err != nil {
		return nil, err
	}
...
func (userDao *UserDao) UpdateUser(id int64, m *models.User) (*models.User, error) {
	...
	res, err := userDao.sqlClient.DB.Exec(updateQuery, m.Username, id)
	if err != nil {
		return nil, err
	}
